...
namespace ORB_SLAM2
{
          ...
          mnLastRelocFrameId(0)
    {
        ...
        int nFeatures = fSettings["ORBextractor.nFeatures"]; // 特征数
        float fScaleFactor = fSettings["ORBextractor.scaleFactor"]; // 金字塔层间比例
        int nLevels = fSettings["ORBextractor.nLevels"]; // 金字塔层数
        int fIniThFAST = fSettings["ORBextractor.iniThFAST"]; // 分块提取 ORB 特征点 初始阈值
        int fMinThFAST = fSettings["ORBextractor.minThFAST"]; // 最小阈值
        ...
        if(mCurrentFrame.N>500)
        {
            ...
            for(int i=0; i<mCurrentFrame.N;i++)
            {
                ...
                if(z>0)
                {
                    ...
                    // 地图点与关键帧的相互关联
                    pNewMP->AddObservation(pKFini,i); // 地图点被关键帧观测到
                    pKFini->AddMapPoint(pNewMP,i); // 帧观测到了点
                    pNewMP->ComputeDistinctiveDescriptors(); // 描述子
                    pNewMP->UpdateNormalAndDepth(); // 法线和深度
    ...
    bool Tracking::TrackReferenceKeyFrame()
    {
        ...
        mCurrentFrame.SetPose(mLastFrame.mTcw); // 初值
